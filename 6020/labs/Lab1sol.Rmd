...
```{r}
Faithful <- read.csv("~/Desktop/Faithful.csv")
```   
...
a. What is notable about the sample distributions of `eruptions` and `waiting`?  *These distributions are bi-modal.*
  
b. Does there appear to be a linear relationship between the time between eruptions and the length of the eruption?  *Yes, the length of the eruption appears to have a positive linear relationship with the time since the last eruption.*
    
  2. Lets check the assumptions for a SLR model.
  
    a.  Are the observations independent?  *This assumption may be violated if observations were taken from consecutive eruptions.*
...
c. Knit your document to see the output from the R code from (b).  Is the homogeneity of variance assumption met?  Is the normality assumption met?  *There are some patterns in the residuals by fitted values plot that may indicate that the homogeneity of variance assumption does not hold.  This may be due to dependence in the data.  However, most points on the Q-Q plot lie within the confidence bands.  The normality assumption seems reasonable.*
...
```
  ...
  *estimated expected length of eruption = $-1.87 + 0.076 \times$ time since last eruption*
  
  5. Interpret the $R^2$ value of this model.
  
  *81 percent of the variability in the estimated duration of an eruption can be explained by its linear relationship with the time since the last eruption.*
  
  6. Is there a significant linear relationship between expected `eruptions` and `waiting`?
  
  *The p-value associated with the test $\beta_1 = 0$ is very small.  So, yes there appears to be a significant linear relationship between the expected length of an eruption and `waiting`.*
  
  7. What is the value of $\hat\beta_1$?  Interpret $\hat\beta_1$ in the context of this analysis.
  
  *$\hat\beta_1$ = 0.076. $\hat\beta_1$ is the estimated increase in the expected duration of an eruption with every extra minute in the time since the last eruption.*
